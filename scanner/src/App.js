...
import logo from "./assets/logo_indaba_white@4x.png";

import Accepted from "./Accepted";
import { AcceptedTag, RefusedTag, AllreadyAcceptedTag } from "./Tags";
import { Loader } from "@mantine/core";

const url = "https://192.168.0.178:8443/api/v1/postData";
...
export default function App() {
  ...
  const handleScann = async (e) => {
    if (e) {
      ...
      setClassNameStyle(
        classNameStyles[[200, 300, 404].indexOf(response.status)]
      );
    }
  };
  const handleError = (e) => {
  ...
  return (
    <div>
      <center>
        <img
          alt="logo"
          style={{ zoom: "0.6", margin: "20px", width: 100 }}
          src={logo}
        />
      </center>
      <Grid container spacing={2}>
        <Grid item xs={6}>
          ...
          <Button
            startIcon={cameraOn ? <VideocamOffIcon /> : <VideocamIcon />}
            variant="outlined"
            style={{ margin: "7px" }}
            onClick={() => {
